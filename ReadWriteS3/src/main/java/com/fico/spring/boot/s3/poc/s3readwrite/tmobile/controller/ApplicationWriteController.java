...
{
    Logger logger =  LoggerFactory.getLogger(ApplicationWriteController.class);

    @Autowired
    ApplicationWriteService appWriteService;

    @PostMapping(path = "/write/{awsRegionName}", consumes = MediaType.APPLICATION_XML_VALUE)
    public ResponseEntity<String> writeS3Storage(@PathVariable String awsRegionName, @RequestBody String appReq)
    {
        logger.info("Entered");
        ...
        try
        {
            ...
            lv_map_reqInfo = appWriteService.appSubmitService(lv_om_result);
            ...
            {
                lv_om_result = null;
                lv_om_result = appWriteService.writeS3OnBucket(lv_map_reqInfo);
            }
...
}//End of class ApplicationSubmitController
